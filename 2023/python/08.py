...
def pattern_divide_func(pattern):
  for i in reversed(range(2, len(pattern) // 2)):
    if len(pattern) % i == 0:
      pat1 = pattern[0:i + 1]
      pats = [
          pattern[j * i: (j + 1) * i + 1] for j in range(len(pattern) // i)
      ]
...
def follow_network_ghost(pattern, network):
  ...
  for i in range(len(pattern)):
    if all(
        [
            i in occ_val[0].keys() for occ_val in occ_vals
        ]
    ):
      beep[i] = [
          (occ_val[0][i], occ_val[2]) for occ_val in occ_vals
      ]
  res = float("inf")
  for key, val in beep.items():
    for x in val:
      print([
          ((x[0] - y[0]) % y[1]) == 0 for y in val
      ])
      if all([
          ((x[0] - y[0]) % y[1]) == 0 for y in val
      ]):
